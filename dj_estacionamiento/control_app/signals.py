# Este archivo se encarga de manejar las señales que se generan en el sistema
...
def procesar_imagen(sender, instance, created, **kwargs):

    print("Se ha guardado una imagen")
    print(f"Created: {created}")
    print(f"Instance: {instance}")
    print(f"Instance.id: {instance.id}")
    print(f"Instance.foto: {instance.foto}")
    if created:

        imagen = cv2.imread(
            "/Users/hernanr/myCode/Robotica/EstacionamientoESP32/dj_estacionamiento/media/vigilancia/picture3.jpg"
        )  # leer imagenro
        ...
        # Procesar la imagen
        # leer placa
        # guardar en la tabla de accesos
        Accesos.objects.get_or_create(foto_id=instance.id, placa=lectur)
        # acceso.save()
    else:
        print("No se ha creado una nueva imagen")
